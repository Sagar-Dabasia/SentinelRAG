...
| RSK-019 | Linter does not support Python 3.14 target | Technical | Low | Low | Low | Ruff v0.5.5 does not support `py314` target-version. Silently reverting to closest supported target `py313`. | Project maintainer | Open | `pyproject.toml` retains `py313` target pending tool release. |
