...
* **Decision:** We use `httpx.MockTransport` coupled with standard library `asyncio.run()` and a custom `httpx.AsyncByteStream` to test provider HTTP streams. We do not patch `AsyncClient.stream` or `AsyncClient.send`.
...
* **Security consequences:** Ensures tested paths do not intentionally contact a real provider, avoiding test contamination. It does not guarantee that all present or future application paths can never leak data.
